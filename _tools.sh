...
function _pre_deploy_monitoring_stack () {
    ...
    kubectl get pods -n kube-system \
        ...
        xargs kubectl delete po -n kube-system

    docker cp kind-control-plane:/etc/kubernetes/pki/etcd/ca.crt .
    docker cp kind-control-plane:/etc/kubernetes/pki/etcd/peer.crt .
    docker cp kind-control-plane:/etc/kubernetes/pki/etcd/peer.key .
...
function _build_cluster () {
    ...
    [ "${enable_monitoring}" == "1" ] && _pre_deploy_monitoring_stack && _deploy_monitoring
